...
if __name__ == "__main__":
    ...
    try:
        print("=" * 80)
        print("[TEST MODE] analyze_node 개선 버전 실행")
        print("=" * 80)
        
        dummy_state = {"rfp_text": ""} 
        result_update = analyze_node(dummy_state)
        
        if "analyzed_json" in result_update and "error" not in result_update["analyzed_json"]:
            result_json = result_update['analyzed_json']
            ...
            # 각 task의 instruction 샘플 출력
            print("\n[작업 배분 샘플]")
            for task_name in ["agency_intro", "research_goal"]:
                if task_name in result_json.get("tasks", {}):
                    task = result_json["tasks"][task_name]
                    print(f"\n{task_name}:")
                    print(f"  instruction: {task.get('instruction', 'N/A')[:200]}...")
                    print(f"  key_points: {task.get('key_points', [])}")
            
            # 파일 저장
            output_file = "debug_analysis_result_improved.json"
            with open(output_file, "w", encoding="utf-8") as f:
                json.dump(result_json, f, indent=2, ensure_ascii=False)
            print(f"\n[저장 완료] {output_file}")
            
        else:
            print("[실패] 분석 결과 없음 또는 오류 발생")
            if "error" in result_update.get("analyzed_json", {}):
                print(f"  에러 메시지: {result_update['analyzed_json']['error']}")
