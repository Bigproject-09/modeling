...
"""
PPT 자동 생성 v6.1 - SlideState 구조 대응
- LLM 완전 자유 디자인
- SlideState의 image_path, image_position 활용
- 이미지는 슬라이드당 1개 (SlideState 레벨)
"""
import os
import json
import base64
...
THEMES = {
    "ocean": {
        "primary": "#003D5B", "secondary": "#0077B6", "accent": "#EF476F",
        "text": "#1E1E1E", "light": "#6B7280", "bg": "#E8F4F8", "card": "#FFFFFF"
    }
}
...
def generate_ppt(state: Dict, output_dir: str, title: str = None) -> Dict:
    """메인 함수"""
    try:
        ...
        print(f"\n[PPT 생성 - LLM 자유 디자인] {title}")
        print(f"  슬라이드: {len(slides)}개")
        
        # 이미지 정보 확인
        img_count = sum(1 for s in slides if s.get('image_path'))
        if img_count > 0:
            print(f"  이미지: {img_count}개")
        
        # LLM이 디자인
        structure = generate_creative_structure(slides, title, theme)
        
        # PPTX 렌더링
        os.makedirs(output_dir, exist_ok=True)
        path = os.path.join(output_dir, f"PPT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pptx")
        render_pptx(structure, path, theme)
        
        print(f"  ✓ 완료: {path}\n")
        return {"ppt_path": path, "theme": "ocean"}
...
def prepare_slide_data(slides: List[dict]) -> List[dict]:
    """슬라이드 데이터 준비 - LLM에게 전달"""
    result = []
    for slide in slides:
        slide_copy = {
        ...
        # 항목들
        for item in slide.get("items", []):
            slide_copy["items"].append({
                "subtitle": item.get("subtitle", ""),
                "content": item.get("content", "")
            })
        
        # 이미지 정보 (SlideState 레벨)
        has_image = bool(slide.get("image_path"))
        slide_copy["has_image"] = has_image
        if has_image:
            slide_copy["image_position"] = slide.get("image_position", "right")
            slide_copy["image_request"] = slide.get("image_request", "")[:50]  # 간략히
...
def generate_creative_structure(slides: List[dict], title: str, theme: Dict) -> Dict:
    """LLM이 창의적으로 디자인"""
    try:
        api_key = os.environ.get("GEMINI_API_KEY")
        if not api_key:
            return generate_fallback_structure(slides, title, theme)
        
        client = genai.Client(api_key=api_key)
        
        # 데이터 준비
        slide_data = prepare_slide_data(slides)
        
        prompt = f"""당신은 세계 최고의 프레젠테이션 디자이너입니다.
**NotebookLM PDF 스타일**의 창의적이고 전문적인 슬라이드를 디자인하세요.
...
**색상:**
Primary: {theme['primary']}, Secondary: {theme['secondary']}, Accent: {theme['accent']}
Text: {theme['text']}, Light: {theme['light']}, BG: {theme['bg']}, Card: {theme['card']}

**슬라이드 크기:** 10인치 × 7.5인치
**단위:** 인치 (inch)

**도형 종류:**
- rectangle, rounded_rectangle, circle, arrow, line

**레이아웃 전략:**
1. **이미지 없음**: simple_list, kpi_cards, timeline, diagram
2. **이미지 있음** (has_image: true):
   - **image_position: "right"** → 좌측 텍스트(5인치) | 우측 이미지(4인치)
   - **image_position: "bottom"** → 상단 텍스트 | 하단 이미지
   - **image_position: "left"** → 좌측 이미지 | 우측 텍스트

**이미지 배치 규칙:**
- "right": x=5.8~9.5, y=1.5~6.5, width=3.5~3.7, height=2.5~4.5
- "bottom": x=1.5~8.5, y=4.8~7.0, width=4.0~7.0, height=1.8~2.2
- "left": x=0.7~4.2, y=1.5~6.5, width=3.5, height=2.5~4.5
- 텍스트와 최소 0.3인치 간격 유지
  ...
  "slides": [
    {{
      ...
      "elements": [
        {{"type": "gradient_bg", "x": 0, "y": 0, "width": 10, "height": 7.5,
          "color1": "{theme['primary']}", "color2": "{theme['secondary']}"}},
        {{"type": "shape", "shape": "rectangle", "x": 0, "y": 0, "width": 0.12, "height": 7.5,
          "fill_color": "{theme['accent']}", "line_color": "none"}},
        {{"type": "text", "x": 1.2, "y": 2.8, "width": 7.6, "height": 2.0,
          "text": "{title}", "font_size": 42, "bold": true, "color": "#FFFFFF", "align": "left"}}
      ]
    }},
    {{
      "slide_number": 1,
      ...
      "has_image": false,
      "elements": [
        {{"type": "background", "x": 0, "y": 0, "width": 10, "height": 7.5, "color": "{theme['bg']}"}},
        {{"type": "line", "x": 0.5, "y": 1.05, "width": 9.0, "height": 0.025, "color": "{theme['secondary']}"}},
        {{"type": "text", "x": 0.5, "y": 0.4, "width": 8.5, "height": 0.6,
          "text": "제목", "font_size": 24, "bold": true, "color": "{theme['primary']}"}},
        {{"type": "shape", "shape": "rounded_rectangle", "x": 0.9, "y": 1.5, "width": 8.2, "height": 1.2,
          "fill_color": "{theme['card']}", "line_color": "{theme['secondary']}", "line_width": 1.5}},
        {{"type": "text", "x": 1.15, "y": 1.7, "width": 7.7, "height": 0.9,
          "text": "내용", "font_size": 12, "color": "{theme['text']}"}}
      ]
    }},
    {{
      "slide_number": 2,
      ...
      "elements": [
        ...
        {{"type": "shape", "shape": "rounded_rectangle", "x": 0.7, "y": 1.5, "width": 4.8, "height": 2.5,
          "fill_color": "{theme['card']}", "line_color": "{theme['secondary']}", "line_width": 1.5}},
        {{"type": "text", "x": 1.0, "y": 1.7, "width": 4.2, "height": 2.2,
          "text": "좌측 텍스트", "font_size": 12, "color": "{theme['text']}"}},
        
        {{"type": "image", "x": 6.0, "y": 1.5, "width": 3.5, "height": 3.0,
          "slide_number": 2}}
      ]
    }}
  ]
...
"""
        ...
        # 검증 및 이미지 경로 매핑
        if validate_structure(structure, len(slides)):
            print(f"  ✓ LLM 디자인 생성 ({len(structure['slides'])}개)")
            map_image_paths(structure, slides)
            return structure
        else:
            print(f"  ! LLM 검증 실패, 폴백 사용")
            return generate_fallback_structure(slides, title, theme)
...
def map_image_paths(structure: Dict, slides: List[dict]):
    """이미지 경로 매핑"""
    for slide_data in structure.get('slides', []):
        slide_num = slide_data.get('slide_number', 0)
        
        if slide_num == 0:  # 표지 스킵
            continue
        
        # 원본 슬라이드 찾기
        if slide_num - 1 < len(slides):
            original = slides[slide_num - 1]
            img_path = original.get('image_path', '')
            ...
            if img_path:
                ...
                for elem in slide_data.get('elements', []):
                    if elem.get('type') == 'image':
                        ...
                        print(f"    슬라이드 {slide_num}: 이미지 매핑 완료")


def validate_structure(structure: Dict, expected: int) -> bool:
    """검증"""
    if not structure or 'slides' not in structure:
        return False
    if len(structure['slides']) != expected + 1:
        return False
    ...
    for slide in structure['slides']:
        for elem in slide.get('elements', []):
            ...
            if x < 0 or y < 0 or x + w > 10.1 or y + h > 7.6:
                print(f"  ! 슬라이드 벗어남: x={x}, y={y}, w={w}, h={h}")
                return False
    
    return True


def generate_fallback_structure(slides: List[dict], title: str, theme: Dict) -> Dict:
    """폴백 - 심플한 레이아웃"""
    structure = {"slides": []}
    ...
    for idx, slide in enumerate(slides, 1):
        elements = [
            ...
            {"type": "line", "x": 0.5, "y": 1.05, "width": 9.0, "height": 0.025, "color": theme['secondary']},
            {"type": "text", "x": 0.5, "y": 0.4, "width": 8.5, "height": 0.6,
             "text": slide.get('title', ''), "font_size": 24, "bold": True, "color": theme['primary']}
        ]
        
        has_image = bool(slide.get('image_path'))
        img_pos = slide.get('image_position', 'right')
        
        # 텍스트 영역 조정
        if has_image and img_pos == "right":
            text_width = 4.8
            text_x = 0.7
        else:
            text_width = 8.2
        ...
        for item in slide.get('items', []):
            ...
            subtitle = item.get('subtitle', '')
            content = item.get('content', '')
            h = 1.0 if len(subtitle + content) < 150 else 1.4
            h = min(h, 6.8 - y)
            ...
            elements.append({
                ...
                "x": text_x, "y": y, "width": text_width, "height": h,
                "fill_color": theme['card'], "line_color": theme['secondary'], "line_width": 1.5
            })
            ...
            elements.append({
                ...
                "x": text_x, "y": y, "width": 0.06, "height": h,
                "fill_color": theme['accent'], "line_color": "none"
            })
            ...
            if subtitle:
                elements.append({
                    "type": "text", "x": text_x + 0.25, "y": y + 0.15, "width": text_width - 0.4, "height": 0.3,
                    "text": subtitle, "font_size": 16, "bold": True, "color": theme['primary']
                })
            
            if content:
                elements.append({
                    "type": "text", "x": text_x + 0.25, "y": y + 0.15 + (0.35 if subtitle else 0),
                    "width": text_width - 0.4, "height": h - 0.3 - (0.35 if subtitle else 0),
                    "text": content[:500], "font_size": 12, "color": theme['text']
                })
        ...
        if has_image:
            if img_pos == "right":
                elements.append({
                    "type": "image", "x": 6.0, "y": 1.5, "width": 3.5, "height": 3.0,
                    "slide_number": idx, "image_path": slide['image_path']
                })
...
def render_pptx(structure: Dict, path: str, theme: Dict):
    """PPTX 렌더링"""
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7.5)
    ...
    for slide_data in structure['slides']:
        ...
        for elem in slide_data['elements']:
            try:
                ...
                if t == 'gradient_bg':
                    render_gradient(slide, elem)
                elif t == 'background':
                    render_bg(slide, elem)
                elif t == 'shape':
                    render_shape(slide, elem)
                elif t == 'line':
                    render_line(slide, elem)
                elif t == 'text':
                    render_text(slide, elem)
                elif t == 'image':
                    render_image(slide, elem)
            except Exception as e:
                print(f"  ! 요소 실패 ({t}): {e}")
                continue
...
def render_gradient(slide, e):
    bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(10), Inches(7.5))
    bg.fill.solid()
    bg.fill.fore_color.rgb = hex_to_rgb(e['color1'])
    bg.line.fill.background()


def render_bg(slide, e):
    bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(10), Inches(7.5))
    bg.fill.solid()
    bg.fill.fore_color.rgb = hex_to_rgb(e['color'])
    bg.line.fill.background()


def render_shape(slide, e):
    shape_map = {
    ...
    shape = slide.shapes.add_shape(
        shape_map.get(e.get('shape', 'rectangle'), MSO_SHAPE.RECTANGLE),
        Inches(e['x']), Inches(e['y']), Inches(e['width']), Inches(e['height'])
    )
    
    fill = e.get('fill_color')
...
def render_line(slide, e):
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE,
                                  Inches(e['x']), Inches(e['y']),
                                  Inches(e['width']), Inches(e['height']))
    line.fill.solid()
    line.fill.fore_color.rgb = hex_to_rgb(e['color'])
    line.line.fill.background()


def render_text(slide, e):
    tb = slide.shapes.add_textbox(
        Inches(max(0, min(e['x'], 9.5))),
        Inches(max(0, min(e['y'], 7.0))),
        Inches(max(0.3, min(e['width'], 10 - e['x']))),
        Inches(max(0.2, min(e['height'], 7.5 - e['y'])))
    )
    ...
    p = tf.paragraphs[0]
    p.text = e.get('text', '')
    p.font.size = Pt(e.get('font_size', 12))
    p.font.bold = e.get('bold', False)
    p.font.color.rgb = hex_to_rgb(e.get('color', '#000000'))
...
def render_image(slide, e):
    ...
    try:
        ...
        x = max(0, min(e['x'], 9))
        y = max(0, min(e['y'], 7))
        w = max(1, min(e['width'], 10 - x))
        h = max(1, min(e['height'], 7.5 - y))
        
        slide.shapes.add_picture(img_path, Inches(x), Inches(y), width=Inches(w), height=Inches(h))
        print(f"  ✓ 이미지 추가: {os.path.basename(img_path)}")
...
def test():
    ...
    test_slides = [
        {
            "page_number": 1,
            "title": "핵심 기술 1: 초고해상도 모델링",
            "section": "기술",
            "items": [
                {
                    "subtitle": "기존 모델의 한계",
                    "content": "100km 격자로 중규모 에디(50~200km) 표현 불가\n해양 순환 정확도 저하"
                },
                {
                    "subtitle": "제안 모델의 혁신",
                    "content": "10km 격자로 정밀 계산\n중규모 에디 명시적 계산"
                }
            ],
            "image_request": "ocean eddy diagram",
            "image_position": "right",
            "image_path": "C:/big_project/modeling/data/images/test-image.png"
        }
    ]
