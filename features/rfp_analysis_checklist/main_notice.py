...
def main():
    ...
    if notice_chunks:
        ...
        try:
            ...
            # JSON 파일로 저장
            judgment_path = os.path.join(ANALYSIS_FOLDER, "eligibility_judgment.json")
    ...
    if analysis_chunks:
        ...
        try:
            ...
            # JSON 파일로 저장
            analysis_path = os.path.join(ANALYSIS_FOLDER, "deep_analysis.json")
            with open(analysis_path, 'w', encoding='utf-8') as f:
                json.dump(analysis_result, f, ensure_ascii=False, indent=2)
            print(f"[4단계] 심층분석 완료 -> {os.path.basename(analysis_path)}")
    ...
    print(f"  - 분석 결과: {ANALYSIS_FOLDER}/")
    if notice_chunks:
        print(f"    · eligibility_judgment.json (자격요건 자동 판정)")
    if analysis_chunks:
        print(f"    · deep_analysis.json (공고 심층 분석)")
    print("=" * 60)
